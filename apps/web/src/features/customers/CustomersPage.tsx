  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="overflow-hidden">
        ...
        {bulkError ? (
          <ErrorBand>{toErrorMessage(bulkError, 'Could not change those customers.')}</ErrorBand>
        ) : null}
...
interface ErrorBandProps {
  children: ReactNode
  /** A single affordance, such as "Try again". */
  action?: ReactNode
}

/**
 * A failure as a band across the card rather than a card of its own, which is
 * what the rounding and the side borders are turned off for.
 *
 * Named because this screen shows two of them — the list failing to load, and a
 * bulk action failing to apply — and two copies of the same six classes in one
 * file is where a drift starts.
 */
function ErrorBand({ children, action }: ErrorBandProps) {
  return (
    <Alert tone="danger" className="items-center rounded-none border-x-0 border-t-0" action={action}>
      {children}
    </Alert>
  )
}
